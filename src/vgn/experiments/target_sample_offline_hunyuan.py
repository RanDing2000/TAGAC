...
def run(
    ...
    for num_id, curr_mesh_pose_list in enumerate(os.listdir(test_mesh_pose_list)):
        ...
        targ_obj = Path(scene_dir) / 'reconstruction' / 'targ_obj.ply'
        if not os.path.exists(targ_obj):
            continue

        if scene_name not in gen3dsr_scene_name_list:
            continue
        ...
        grasps, scores, timings["planning"], visual_dict = grasp_plan_fn(state, scene_mesh, hunyun2_flag=True, scene_name=scene_name)
