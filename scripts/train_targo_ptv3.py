...
def main(args):
    ...
    if not args.shape_completion:
        sc_net = None
    trainer = create_trainer(net, sc_net, optimizer, loss_fn, metrics, device, args.input_points, args.net)
    evaluator = create_evaluator(net, sc_net, loss_fn, metrics, device, args.input_points, args.net)
...
def prepare_batch(batch, device, model_type="targo_ptv3"):
    ...
    if model_type == "ptv3_scene":
        # For ptv3_scene, only return scene point cloud
...
def create_trainer(net, sc_net, optimizer, loss_fn, metrics, device, input_points, model_type="targo_ptv3"):
    def _update(_, batch):
...
def create_evaluator(net, sc_net, loss_fn, metrics, device, input_points, model_type="targo_ptv3"):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--net", default="targo_ptv3", choices=["targo_ptv3", "a"], 
    ...
    parser.add_argument("--logdir", type=Path, default="/usr/stud/dira/GraspInClutter/grasping/train_logs_targo_ptv3")
    ...
    parser.add_argument("--use_complete_targ", type=str2bool, default=False, help="If true, use complete target mesh from preprocessed data")
    ...
    parser.add_argument("--shape_completion", type=str2bool, default=True, help="If true, use shape completion")
